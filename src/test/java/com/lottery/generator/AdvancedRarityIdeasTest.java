...
class AdvancedRarityIdeasTest {
    ...
    static final class GapModel {final double[] lo=new double[4],hi=new double[4];static GapModel from(List<List<Integer>>h,double cov){GapModel m=new GapModel();double t=(1-cov)/2;for(int p=0;p<4;p++){final int pp=p;double[]v=h.stream().mapToDouble(d->gaps(d)[pp]).toArray();m.lo[p]=quantile(v,t);m.hi[p]=quantile(v,1-t);}return m;}int outside(List<Integer>x){int[] g=gaps(x);int n=0;for(int i=0;i<4;i++)if(g[i]<lo[i]||g[i]>hi[i])n++;return n;}}
